...
def price_calc(i):
    ...
    purchase_L.config(text=f"${amount*lemon_price:.2f}")
...
price1 = ttk.Label(root, text=f"${lemon_price:.2f}")
price1.grid(row=2, column=3, padx=10)

price2 = ttk.Label(root, text=f"${icecube_price:.2f}")
price2.grid(row=3, column=3, padx=10)

price3 = ttk.Label(root, text=f"${lemonade_price:.2f}")
...
quantity2 = ttk.Entry(root, width=5, textvariable=count)
quantity2.grid(row=3, column=1, padx=5)

quantity3 = ttk.Entry(root, width=5, textvariable=count)
